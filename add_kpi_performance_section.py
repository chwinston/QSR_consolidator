"""
Add KPI Performance Section to Measurement Approaches Document
"""

import pandas as pd

# Load the data
df = pd.read_csv('analysis_results/kpi_measurement_quality_analysis.csv')
...
for category in sorted(df['kpi_category'].unique()):
    ...
    section.append(f"**Average Target**: {cat_data['kpi_target'].mean():.1f}\n") if cat_data['kpi_target'].notna().any() else section.append(f"**Average Target**: N/A\n")
    section.append(f"**Average Actual**: {cat_data['kpi_actual'].mean():.1f}\n") if cat_data['kpi_actual'].notna().any() else section.append(f"**Average Actual**: N/A\n")
    section.append(f"**Average Delta**: {cat_data['kpi_delta'].mean():.1f}\n\n") if cat_data['kpi_delta'].notna().any() else section.append(f"**Average Delta**: N/A\n\n")
    ...
    section.append(f"- On Target: {len(cat_data[cat_data['kpi_delta'] == 0])} of {len(cat_data)}\n\n")
...
# Find where to insert (after section 2 - Detailed Breakdown)
# Insert before the first "### Adoption Rate" which starts section 2
...
if insert_point != -1:
    ...
    toc_addition = """
3. [KPI Performance Summary by Category](#3-kpi-performance-summary-by-category)
...
"""
    ...
    print("✓ Added KPI Performance Summary section")
